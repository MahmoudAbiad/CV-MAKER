...
def _strip_schema_defaults(obj):
    """يشيل مفتاح 'default' من الـ JSON schema بشكل متداخل، لأن Gemini
    (نسخة protobuf) ما بتدعم هالمفتاح وبترفضه بخطأ Unknown field."""
...
# --- Schemas لـ Gemini مع قيم افتراضية لمنع الأخطاء ---
class Experience(BaseModel):
    title: str = ""
    company: str = ""
    dates: str = ""
...
async def process_bio(message: Message, state: FSMContext):
    if len(message.text.strip()) < 5:
        await message.answer("⚠️ يرجى إرسال بيانات كافية (الاسم، الخبرات، أو المهارات) لإنشاء السيرة الذاتية.")
        return
