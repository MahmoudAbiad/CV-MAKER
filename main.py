...
# --- تحميل الخطوط العربية تلقائياً إن لم تكن موجودة ---
def download_arabic_fonts():
    fonts = {
        "Cairo-Regular.ttf": "https://raw.githubusercontent.com/google/fonts/main/ofl/cairo/static/Cairo-Regular.ttf",
        "Cairo-Bold.ttf": "https://raw.githubusercontent.com/google/fonts/main/ofl/cairo/static/Cairo-Bold.ttf"
    }
    ...
    for font_name, url in fonts.items():
        if not os.path.exists(font_name):
            try:
                ...
                print(f"⚠️ فشل تحميل الخط {font_name}: {e}")
...
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "super_secret_token_123").strip()
...
class CVData(BaseModel):
    is_arabic: bool = Field(default=True, description="True if the text is mostly Arabic")
    full_name: str = ""
    email: str = ""
    phone: str = ""
    location: str = ""
...
def _sanitize_schema_for_gemini(schema: dict) -> dict:
    """
    Gemini (نسخة protobuf) ما بتدعم مفاتيح JSON Schema القياسية متل:
    $defs / definitions / $ref / title / default / additionalProperties
    هاي الدالة بتعمل inline لكل $ref (تحل مكانها التعريف الفعلي من $defs)
    وبتشيل باقي المفاتيح غير المدعومة بشكل متداخل (recursive).
    """
    defs = {}
    defs.update(schema.pop("$defs", {}) or {})
    defs.update(schema.pop("definitions", {}) or {})

    UNSUPPORTED_KEYS = {"title", "default", "additionalProperties", "$defs", "definitions"}

    def resolve(node):
        if isinstance(node, dict):
            # نمط pydantic v2 الشائع: {"allOf": [{"$ref": "#/$defs/X"}], ...extra}
            if "allOf" in node and len(node["allOf"]) == 1 and "$ref" in node["allOf"][0]:
                ref_name = node["allOf"][0]["$ref"].split("/")[-1]
                merged = copy.deepcopy(defs.get(ref_name, {}))
...
def check_missing_fields(cv_data: dict) -> List[str]:
    missing = []
    if not cv_data.get("full_name"):
        missing.append("الاسم الكامل")
    if not cv_data.get("email") and not cv_data.get("phone"):
        missing.append("وسيلة تواصل (هاتف أو إيميل)")
    if not cv_data.get("summary"):
        missing.append("نبذة مهنية مختصرة عنك")
    if not cv_data.get("experience"):
        missing.append("خبرة عملية واحدة على الأقل (المسمى، الشركة، التواريخ)")
    if not cv_data.get("education"):
        missing.append("مؤهل تعليمي واحد على الأقل")
    if not cv_data.get("skills"):
        missing.append("قائمة مهارات")
    return missing
...
async def extract_cv_data(user_text: str) -> dict:
    ...
    If any piece of information is not mentioned by the user, leave that field
    empty (empty string or empty list) instead of guessing or inventing it.
...
def generate_pdf(data: dict, output_path: str):
    ...
    valid_contacts = [p for p in contact_parts if p]
    if valid_contacts:
        story.append(Paragraph(ar_text(" | ".join(valid_contacts)), contact_style))
...
def generate_docx(data: dict, output_path: str):
    ...
    valid_contacts = [p for p in contact_parts if p]
    if valid_contacts:
        p_contact.add_run(" | ".join(valid_contacts)).font.size = Pt(9.5)
...
async def start_cmd(message: Message, state: FSMContext):
    welcome_text = (
        ...
        "اكتب لي بأسلوبك الحر أي معلومات عندك عن نفسك: اسمك، خبراتك، دراستك، مهاراتك...\n"
        "ما في داعي لصيغة معينة أو ترتيب — اكتب متل ما بيحلولك.\n\n"
        "لو نقصت أي معلومة مهمة، رح أخبرك بالضبط شو ناقص وتقدر تبعتلي إياها بأي وقت.\n\n"
        "🚀 بمجرد ما تكتمل المعلومات، بولّد فوراً ملف PDF مجاني ومتوافق مع ATS."
    )
    await message.answer(welcome_text)
    await state.set_state(CVFlow.waiting_for_bio)
    await state.update_data(bio_text="")

@dp.message(CVFlow.waiting_for_bio, F.text)
async def process_bio(message: Message, state: FSMContext):
    if len(message.text.strip()) < 2:
        await message.answer("⚠️ ابعتلي معلومات أكتر شوي حتى أقدر أساعدك.")
        return

    data = await state.get_data()
    prev_text = data.get("bio_text", "")
    combined_text = f"{prev_text}\n{message.text}".strip() if prev_text else message.text

    status_msg = await message.answer("⏳ جاري تحليل بياناتك...")
    try:
        cv_data = await extract_cv_data(combined_text)
        missing = check_missing_fields(cv_data)

        if missing:
            await state.update_data(bio_text=combined_text)
            missing_list = "\n".join(f"• {m}" for m in missing)
            await status_msg.edit_text(
                "📝 لسا ناقصة بعض المعلومات، ابعتها بأي رسالة وراح أكمّل السيرة الذاتية:\n\n" + missing_list
            )
            return
        ...
        logging.error(f"Error processing CV: {e}", exc_info=True)
        await status_msg.edit_text(f"❌ حدث خطأ أثناء المعالجة: {str(e)[:120]}")
...
async def handle_receipt(message: Message, state: FSMContext):
    ...
    try:
        ...
        logging.error(f"Error handling receipt: {e}", exc_info=True)
        await status_msg.edit_text("❌ تعذر تدقيق الإيصال، يرجى المحاولة مرة أخرى.")
...
async def on_startup(bot: Bot):
    ...
    for attempt in range(5):
        try:
            ...
            print(f"⏳ Flood control, retrying in {e.retry_after}s...")
            await asyncio.sleep(e.retry_after + 1)
        except Exception as e:
            print(f"⚠️ Failed to set webhook (attempt {attempt + 1}): {e}")
            await asyncio.sleep(2)
    print("❌ Could not set webhook after multiple attempts.")
